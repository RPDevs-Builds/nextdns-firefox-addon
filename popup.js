...
async function initializeApp() {
  ...
  if (masterToggle && tldsToggle && logActionsToggle && descToggle && profileNotesToggle && filterToggle && featuresDiv) {
    ...
    profileNotesToggle.onchange = async (e) => {
      await browser.storage.sync.set({ webGuiProfileNotes: e.target.checked });
    };
  }
  ...
  if (descViewerBtn) {
    descViewerBtn.onclick = () => {
      browser.tabs.create({ url: "viewer.html?tab=domains" });
    };
  }

  const profileNotesViewerBtn = document.getElementById("profile-notes-viewer-btn");
