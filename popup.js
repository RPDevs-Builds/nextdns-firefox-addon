...
function initTabNavigation() {
    ...
    if (settingsSubNav) {
        settingsSubNav.onclick = (e) => {
            ...
            if (btn) {
                ...
                if (target === 'schedules') loadRules();
                if (target === 'audit') { 
                    document.getElementById('audit-score-ring').textContent = "--";
    ...
    if (runAuditBtn) {
        runAuditBtn.onclick = async () => {
            ...
            runAuditBtn.textContent = "🛡️ Run Security Scan";
        };
    }
    ...
    // Sub-nav for Dashboard (Phase 4.1)
    const dashboardSubNav = document.getElementById('dashboard-sub-nav');
    if (dashboardSubNav) {
        dashboardSubNav.onclick = (e) => {
            const btn = e.target.closest('.sub-tab-btn');
            if (btn) {
                dashboardSubNav.querySelectorAll('.sub-tab-btn').forEach(b => b.classList.remove('active'));
                document.querySelectorAll('.dashboard-sub-content').forEach(c => { c.classList.remove('active'); c.style.display = 'none'; });
                
                btn.classList.add('active');
                const target = btn.getAttribute('data-sub');
                const content = document.getElementById(`dashboard-${target}`);
                if (content) { 
                    content.classList.add('active'); 
                    content.style.display = 'block'; 
                }
    ...
    const runDebuggerBtn = document.getElementById('run-debugger-btn');
    if (runDebuggerBtn) {
        runDebuggerBtn.onclick = async () => {
            runDebuggerBtn.disabled = true;
            runDebuggerBtn.textContent = "Analyzing...";
            await runIntelligentDebugger();
            runDebuggerBtn.disabled = false;
            runDebuggerBtn.textContent = "🔍 Analyze This Page";
        };
    }
}
...
async function runSecurityAudit() {
    if (!activeProfile) return alert("No active profile.");
    const res = await browser.runtime.sendMessage({ type: "RUN_AUDIT", profileId: activeProfile });
